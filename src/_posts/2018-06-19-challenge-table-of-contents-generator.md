...
description: "A developer challenge to build a table of contents generator from HTML headers, with multiple solution approaches."
---
...
Want to give it a try? Here are some basic specs in Ruby using RSpec to start. With that said, let's dive in and explore a solution! No peeking until you've given it a try first.
...
For that, we're going to want to start with an idea of a Tree Node. We want to be able to capture the concept of a header tag having potential children, and potentially a parent header.
...
# => [#<Nokogiri::XML::Element:... name="h1" children=[...]>]
```
...
This means we have to ignore it when we're iterating, but it affords us a lot of flexibility in relative depth.
