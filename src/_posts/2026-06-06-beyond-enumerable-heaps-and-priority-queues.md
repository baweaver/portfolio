...
> **Note on `shift`:** You'll occasionally see `Array#shift` described as `O(n)`. MRI uses a shared buffer with a pointer offset, making it amortized `O(1)` (<%= repo_link "benchmark", "src/_code/beyond-enumerable-heaps/bench_shift.rb" %> shows flat ~0.014µs/elem from n=10k to n=400k on Ruby 3.4). The cost in this snippet is the re-sort on every arrival, not the `shift`.
